...
@section('content')
    <div class="flex flex-col w-fit">
        <div class="bg-blue-400 text-center font-bold border-b-1 p-2">Редагування регіону</div>
        <div class="bg-white">
            <form class="flex" action="{{route('root.regions.update', $region->id)}}" method="POST">
                @csrf
                @method('PATCH')
                    ...
                    <div class="table-row-group">
                        <div class="table-row border-b">
                            ...
                            <div class="table-cell p-1">
                                <input type="text" name="name" placeholder="RegionName"
                                       @if(old('name'))
                                           value="{{old('name')}}"
                                       @else
                                           value="{{$region->name}}"
